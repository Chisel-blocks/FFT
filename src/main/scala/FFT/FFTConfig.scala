...
trait FFTConfig[T <: Data]{
  ...
  val n: Int // n-point FFT
  val pipelineDepth: Int
  val lanes: Int // number of input
  val quadrature: Boolean
  val inverse: Boolean // do inverse fft when true
  val unscrambleOut: Boolean //  correct output bit-order, only functional when (n=lanes)
  val unscrambleIn: Boolean // accept srambled input
